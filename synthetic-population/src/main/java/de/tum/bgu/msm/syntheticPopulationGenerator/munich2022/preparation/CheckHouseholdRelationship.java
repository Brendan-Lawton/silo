...
/*    private TableDataSet microDataHousehold;
    ...
    public void run(){
        ...
        for (int household = 1; household <= dataSetSynPop.getHouseholdTable().rowKeySet().size(); household++){
            int hhSize = dataSetSynPop.getHouseholdTable().get(household, "h.size");
    ...
    private void obtainRolesInHousehold(int firstMember, int hhSize){
        for (int person = 0; person < hhSize; person++){
            ...
            int spouseInHousehold = dataSetSynPop.getPersonTable().get(row, "p.partnerInHousehold");
            int relationToHead = dataSetSynPop.getPersonTable().get(row, "p.householdRole");
            int maritalStatus = dataSetSynPop.getPersonTable().get(row, "p.maritalStatus");
            int age = dataSetSynPop.getPersonTable().get(row, "p.age");
            int gender = dataSetSynPop.getPersonTable().get(row, "p.gender");
            if (relationToHead == 3) {
